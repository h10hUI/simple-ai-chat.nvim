...
2. ファイルパスをコードブロックのヘッダから取得（例: ` ```lua:lua/foo.lua ` ）
3. ファイルパスがなければ、現在フォーカスしていた編集バッファを対象とする
4. `vim.diff()` で差分を生成し、差分バッファを新規タブで表示
5. 差分バッファで `<CR>` で適用、`q` でキャンセル

### コードブロックのファイルパス指定例（AI への指示に含める）

```
以下の形式で返してください:
```lua:lua/ai-chat/chat.lua
-- コード
```
```
